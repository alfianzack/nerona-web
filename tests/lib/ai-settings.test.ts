...
describe("getAiSettings", () => {
  ...
  it("reads all keys in a single query", async () => {
    rows({});
    await getAiSettings();
    ...
    expect(where.key.in).toEqual(
      expect.arrayContaining(["ai_model", "ai_price_in", "ai_price_out", "points_per_usd"])
    );
  });
});
...
describe("getAiSettingsView", () => {
  it("returns the model, never a key", async () => {
    rows({ ai_model: "gpt-5" });
    const view = await getAiSettingsView();
    expect(view.model).toBe("gpt-5");
    expect(view).not.toHaveProperty("apiKeyMasked");
    expect(view).not.toHaveProperty("apiKeySet");
  });
